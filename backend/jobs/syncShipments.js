...
const BATCH_SIZE = 500; // cap per run — at high volume, more shipments can be "in flight" than one pass could ever get through
const CONCURRENCY = 20; // parallel DTDC tracking calls in flight at once
...
/**
 * Polls DTDC for shipments that aren't in a terminal state, updates our copy
 * of the status, records any failed delivery attempt (NDR) with its reason,
 * and flags anything stuck too long as needs_review.
 *
 * Processes at most BATCH_SIZE shipments per run — oldest-synced first, so
 * the most overdue ones get checked before newer ones even under a large
 * backlog — with CONCURRENCY DTDC calls in flight at once rather than one at
 * a time. At "lakhs of orders" volume a strictly sequential loop over every
 * non-terminal shipment could easily take longer than the interval between
 * scheduled runs (see server.js); this bounds each run's worst case instead.
 * Runs on a schedule from server.js.
 */
async function syncShipments() {
  ...
  await mapWithConcurrency(shipments, CONCURRENCY, (shipment) => syncOneShipment(sb, shipment));
  console.log(`[syncShipments] Synced ${shipments.length} shipment(s).`);
}

async function syncOneShipment(sb, shipment) {
  try {
